...
func TestSigningKey(t *testing.T) {
	...
	opts.ProviderURLString = "https://auth.sso.dev"
	opts.upstreamConfigs = generateTestUpstreamConfigs("foo-internal.sso.dev")
	opts.RequestSigningPublicKey = string(`-----BEGIN PUBLIC KEY-----
...
-----END PUBLIC KEY-----`)
	...
	rw := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "https://foo.sso.dev/signingkey", nil)
	proxy.Handler().ServeHTTP(rw, req)
	testutil.Equal(t, 200, rw.Code)
	testutil.Equal(t, opts.RequestSigningPublicKey, rw.Body.String())
}
